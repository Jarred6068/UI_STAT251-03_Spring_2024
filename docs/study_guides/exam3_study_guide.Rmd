...
\[ \hat{p} \pm z\times \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \]
  ...
  * **Assumptions**: sampling with replacement, requires large $n$, only valid when
...
\[ \bar{x} \pm t\times \frac{s}{\sqrt{n}} \]
  ...
  * **Assumptions**: sampling with replacement, works for any $n$, assumes population distribution of $x$ is normal
...
**Steps of a sign test:**
  ...
  * where $p_0 $ is the probability of a positive sign
  ...
  3. The test statistic $X$ is the total number of positive signs which follows a binomial distribution $X \sim \text{binom}(n, p_0)$ where $n$ is the total number of signs
...
\[ = 2 \times \left[ 1 - \sum_{k = 0}^{s - 1}\frac{n!}{k!(n-k)!}p_0^k (1-p_0)^{n-k}\right] \] 
  ...
  * for a right-tailed test $H_A: p_0 > p_0$: 
  ...
  \[P(x\geq X| H_0) = 1 - P(x<X| H_0)\]
	
  \[ = 1 - \sum_{k = 0}^{s - 1}\frac{n!}{k!(n-k)!}p_0^k (1-p_0)^{n-k} \]

  * for a left-tailed test $H_A: p_0 < p_0$:
  
  \[P(x\leq X| H_0) = \sum_{k = 0}^{s}\frac{n!}{k!(n-k)!}p_0^k (1-p_0)^{n-k} \] 
