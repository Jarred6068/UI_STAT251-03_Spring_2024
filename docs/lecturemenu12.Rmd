...
The gardener suspects that the plants which received the new fertilizer will tend to be taller than the plants that received only water. They set the null hypothesis to be "no difference in height" which can be thought of as "the fertilizer has no effect on tomato plant growth". 
...
Since the suspicion is that the plants receiving fertilizer will be taller, the alternative hypothesis is
...
df = cbind.data.frame(c('Sample size', 'Mean Height (Fertilized)', 'SD (Fertilized)', 'Mean Height (Water Only)', 'SD (Water Only)', 'Mean Difference (Fertilized - Water Only)', 'SD Difference'),
...
\[ t_{obs} = \frac{\bar{x}_d - \mu_d}{s_d/\sqrt{n}}  = \frac{4.14 - 0}{4.43/\sqrt{40}} = 5.91 \sim t(n-1)\]
...
Using the above formula the gardener calculates a $t$-score of $t_{obs} = 5.91$ with $39$ degrees of freedom. The probability from earlier can be re-framed in the context of the $t$-distribution

\[P(t \geq t_{\text{obs}}| H_0 \ \text{true}) = 1-P(t< t_{\text{obs}}|H_0 \ \text{true}) \]

i.e the probability of observing a $t$ value greater than the one we observed when the true difference is assumed to be $\mu_d = 0$. For the gardener's experiment, this probability is $3.39 \times 10^{-7} \approx 0$, which is very small.


This probability is called the **p-value** and it measures the probability of observing an estimate as or more extreme than the one we observed under the assumption that the null hypothesis is true. So what does this small probability mean? Just as we observed with the coin flip example from last week, it can mean one of two things:

1. We observed something very unusual

2. The assumption underlying the calculation (i.e $\mu_d = 0$ no difference in mean height) is not true.
...
In the plot above, $t_{1-\alpha}$ denotes the boundary for significance in the gardener's test. Any $t$-value greater than $t_{1-\alpha}$ will have a **p-value** less than $\alpha$ and be deemed <i>significant</i>
...
What were the steps involved with this test?
  ...
  * **step 4** - Compute the **$p$-value** - the probability of observing a value of the test statistic that is as extreme or more extreme than the observed value given that the null hypothesis is true
  
  * **step 5** - Make a conclusion about the null hypothesis - we either (i) fail to reject the null hypothesis and conclude that there is not sufficient evidence in favor of the alternative or (ii) reject the null hypothesis in favor of the alternative


With the examples of significance tests we have discussed thus far we have learned two formal significance tests: one concerning a population proportion $p$ and one concerning a population mean $\mu$.

# Lecture 22 Wednesday, March 27th 2024

### Setting up the null and alternative hypothesis

As we stated previously, the null hypothesis is a statement about the value of a population parameter. The alternative hypothesis is a statement about our suspicions regarding the value of that parameter. The alternative hypothesis can be directional such as $H_A: p > p_0$ (called an upper tail test) or $H_A: p < p_0$ (called a lower tailed test). These types of alternative hypotheses are called *one-sided* because they direct our attention to either the lower or upper tail of the distribution of the test statistic. If our suspicion about the parameter is that it differs in either direction or if we are unsure, the alternative hypothesis is $H_A: p \neq p_0$ and the test is called *two-sided* because the suspicion includes both the lower and upper tails of the distribution of the test statistic. 
...
The decision rule stated in terms of the critical value and rejection region is:
...
A = ggplot()+xlim(-4, 4)+theme_bw()+
  ...
  stat_function(fun = dnorm,
                size = 1)+
  geom_vline(xintercept = c(qnorm(0.05/2), qnorm(1-(0.05/2))), size = 2, linetype = 'dashed')+
...
B = ggplot()+xlim(-4, 4)+theme_bw()+
  ...
  stat_function(fun = dnorm,
                size = 1)+
  geom_vline(xintercept = c(qnorm(1-0.05)), size = 2, linetype = 'dashed')+
  geom_text(aes(x = 0, y = 0.15), label = TeX('Do Not Reject $H_0$'), size = 8)+
  #geom_text(aes(x = -3, y = 0.15), label = TeX('Reject $H_0$'), size = 8)+
  geom_text(aes(x = 3, y = 0.15), label = TeX('Reject $H_0$'), size = 8)+
  xlab('Z')+
...
C = ggplot()+xlim(-4, 4)+theme_bw()+
  ...
  stat_function(fun = dnorm,
                size = 1)+
  geom_vline(xintercept = c(qnorm(0.05)), size = 2, linetype = 'dashed')+
  geom_text(aes(x = 0, y = 0.15), label = TeX('Do Not Reject $H_0$'), size = 8)+
  geom_text(aes(x = -3, y = 0.15), label = TeX('Reject $H_0$'), size = 8)+
  #geom_text(aes(x = 3, y = 0.15), label = TeX('Reject $H_0$'), size = 8)+
  xlab('Z')+
...
Notice from the plots above that the alternative hypothesis also impacts how significant a result must be in order to reject the null. Specifically, a result need not be as extreme under the alternative hypotheses $H_A: p > p_0$ and $H_A: p < p_0$ to be statistically significant. This is because when we suspect the direction of the effect we have more information about the parameter which improves our ability to detect a significant result.

# Lecture 23 Friday, March 29th 2024
...
df = cbind.data.frame(`Alternative Hypothesis` = c('$H_A: p \\neq p_0$','$H_A: p > p_0$','$H_A: p < p_0$'),
                      ...
                      `Rejection Region` = c('$-\\infty \\leq Z \\leq Z_{\\alpha/2}$ or $Z_{1-\\alpha/2}\\leq Z \\leq \\infty$','$Z_{1-\\alpha} \\leq Z \\leq \\infty$','$-\\infty \\leq Z \\leq Z_{\\alpha}$'))
...
1. A leading electric vehicle (EV) manufacturer claims that at least $20\%$ of all vehicles sold in a particular region are electric. To verify this claim, a consumer advocacy group randomly surveys $500$ recent vehicle purchases in the region and finds that $115$ of them are electric. Conduct a significance test at the $\alpha = 0.01$ significance level to determine if the proportion of vehicles sold is less than the manufacturer's claim.

*Solution.* Here $n = 500$, $p_0 = 0.20$, and the sample proportion is $\hat{p} = 115/500 = 0.23$. We test $H_0: p = 0.20$ against the lower-tail alternative $H_A: p < 0.20$ at $\alpha = 0.01$.

  * **Check the assumption.** $np_0 = 500(0.20) = 100 \geq 15$ and $n(1-p_0) = 500(0.80) = 400 \geq 15$, so the sample is large enough for the normal approximation.

  * **Test statistic.**
  \[ Z_{obs} = \frac{\hat{p} - p_0}{\sqrt{\frac{p_0(1-p_0)}{n}}} = \frac{0.23 - 0.20}{\sqrt{\frac{0.20(0.80)}{500}}} = \frac{0.03}{0.0179} = 1.68 \]

  * **$p$-value.** For the lower-tail alternative, $p\text{-value} = P(Z \leq 1.68) \approx 0.95$.

  * **Conclusion.** Since the $p$-value $\approx 0.95$ is far greater than $\alpha = 0.01$, we *fail to reject* $H_0$. The observed proportion ($0.23$) is actually above the claimed $0.20$, so there is no evidence that the true proportion of EVs is less than $20\%$.
...
1. A government agency is interested in understanding the average annual income of households in a particular region to inform economic policies. The agency hypothesizes that the mean annual income of households in this region is $\$50,000$. To test this, they collect a random sample of $100$ households from the region and estimate the annual income to be $\$45,323$ with a standard deviation of $\$13,121$. Conduct a hypothesis test at the $\alpha = 0.05$ significance level to determine if the annual income is significantly different than the agency's hypothesis for this region.

*Solution.* Here $n = 100$, $\bar{x} = 45{,}323$, $s = 13{,}121$, and $\mu_0 = 50{,}000$. Because we are asked whether the mean is *different* (in either direction), we test $H_0: \mu = 50{,}000$ against the two-sided alternative $H_A: \mu \neq 50{,}000$ at $\alpha = 0.05$.

  * **Check the assumption.** The sample size $n = 100$ is large, so by the Central Limit Theorem the sampling distribution of $\bar{x}$ is approximately normal.

  * **Test statistic.**
  \[ t_{obs} = \frac{\bar{x} - \mu_0}{s/\sqrt{n}} = \frac{45{,}323 - 50{,}000}{13{,}121/\sqrt{100}} = \frac{-4677}{1312.1} = -3.57 \sim t(99) \]

  * **$p$-value.** For the two-sided alternative, $p\text{-value} = 2 \times P(t \leq -3.57) \approx 0.0006$.

  * **Conclusion.** Since the $p$-value $\approx 0.0006$ is less than $\alpha = 0.05$, we *reject* $H_0$. There is strong evidence that the mean annual household income differs from $\$50{,}000$ (specifically, the data suggest it is lower).

### Relationship between two-sided tests and confidence intervals

Recall the basic idea of a confidence interval. We construct an interval that includes the true value of the parameter $\mu$ or $p$ with specified frequency $100(1-\alpha)\%$. Now consider a two-sided significance test for a population mean. A two-sided test constructs an **acceptance region** around the value of the parameter suggested in the null hypothesis (i.e $\mu_0$). At the $100(1-\alpha)\%$ confidence level, this acceptance region is given by $[\mu_0-t_{1-\alpha/2} \times \frac{s}{\sqrt{n}}, \mu_0 + t_{1-\alpha/2}\times \frac{s}{\sqrt{n}}]$. Values of $\bar{x}$ that are not in this interval seem to be incompatible with the data. Thus when $\bar{x}$ is not in this interval, it is statistically significant and the null hypothesis is rejected. This produces a significance test at $\alpha = 0.05$ significance level.


**Example**

Return to the household income problem above ($\mu_0 = 50{,}000$, $n = 100$, $\bar{x} = 45{,}323$, $s = 13{,}121$, $\alpha = 0.05$). The standard error is $s/\sqrt{n} = 13{,}121/\sqrt{100} = 1312.1$, and with $n-1 = 99$ degrees of freedom the critical value is $t_{99,\ 0.975} \approx 1.984$. The $95\%$ acceptance region around $\mu_0$ is

\[ \left[50{,}000 - 1.984 \times 1312.1,\ \ 50{,}000 + 1.984 \times 1312.1\right] = [47{,}397,\ 52{,}603] \]

The observed sample mean $\bar{x} = 45{,}323$ falls **outside** this interval, so we reject $H_0$ at the $\alpha = 0.05$ level. This is exactly the same decision we reached with the test statistic ($t_{obs} = -3.57$, $p$-value $\approx 0.0006 < 0.05$): a two-sided test rejects $H_0$ precisely when the observed $\bar{x}$ lies outside the $100(1-\alpha)\%$ acceptance region centered at $\mu_0$.
...
\[ Power = P(t > t_{1-\alpha} | H_0 \ \text{Not True}) \]
...
The **type I error** rate is the probability that we reject the null when the null is in fact true (denoted $\alpha$). 

For example, the probability of committing a type I error for a right-tailed (or upper tail) test of a population mean is 
\[ P( t > t_{1-\alpha} | H_0 \ \text{True}) =\alpha \]

Another type of error that can occur is called a **type II error** (denoted $\beta$). This type of error occurs when we fail to reject the null hypothesis but the alternative hypothesis is true.


The probability of committing a type II error for a right-tailed (or upper tail) test of a population mean is 
\[ P( t \leq t_{1-\alpha} | H_0 \ \text{Is not True}) =\beta \]


Looking at the previous example it is easy to see that power is the complement of the type II error rate.

\[ Power = P(t \geq t_{1-\alpha} | H_0 \ \text{Not True}) = 1 - P( t \leq t_{1-\alpha} | H_0 \ \text{Is not True}) \]
\[ = 1-\beta \]

The relationship between the result of a test and the truth underlying the test can be summarized as follows:

```{r, echo=FALSE, warning=F, message=F}

df = rbind.data.frame(x = c('Decision: Reject $H_0$','$\\alpha$', '$1-\\beta$'),
                      y = c('Decision: Fail To Reject $H_0$','$1 - \\alpha$', '$\\beta$'))

kable(df, format = 'html', digits = 2, col.names = c('', '$H_0$ is True', '$H_0$ is False'),
      row.names = F, booktabs = T, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped')
...
This can be visualized in terms the distribution of the test statistic under the null and alternative hypotheses. The plot below shows the relationship between power, type I and type II error rates in a right-tailed test of a population mean conducted at the $\alpha$ significance level. 

```{r, echo=FALSE, warning=F, message=F, fig.height=6, fig.width=12}

dlim_A = function(x, q, mean, sd, test){
  q_a = q
  y = dnorm(x, mean, sd)
...
alpha = 0.05
data = cbind.data.frame(x = seq(-4, 7, 0.001))
ggplot(data = data, aes(x = x, fill = 'grey'))+theme_bw()+
  stat_function(fun = dnorm,
                geom = 'area',
                fill = 'blue',
                alpha = 0.5)+
  ...
  stat_function(fun = dlim_A,
                args = list(q = qnorm(1-alpha)+.05, mean = 3, sd = 1, test = 'lower.tail'),
                geom = 'area',
                fill = 'green',
                alpha = 0.9)+
  stat_function(fun = dlim_0,
                args = list(q = qnorm(1-alpha)-0.05, test = 'upper.tail'),
                geom = 'area',
                fill = 'orange',
                alpha = 0.9)+
  ...
  stat_function(fun = dnorm,
                ...
                size = 1.5,
                linetype = 'dashed')+
  geom_segment(aes(x = qnorm(1-alpha), y = 0, xend = qnorm(1-alpha), yend = 0.4), size = 1, linetype = 'dotted')+
  geom_text(aes(x = 0, y = dnorm(0)+0.05), label = 'Distribution Under H0', size = 6)+
  geom_text(aes(x = 3, y = dnorm(3, 3, 1)+0.05), label = 'Distribution Under HA', size = 6)+
  geom_text(aes(x = 1.3, y = dnorm(1.3, 3, 1)/2), label = TeX('$\\beta$'), size = 7)+
  geom_text(aes(x = qnorm(1-alpha)+.4, y = dnorm(qnorm(1-alpha)+0.4)/2), label = TeX('$\\alpha$'), size = 7)+
  geom_text(aes(x = 3, y = dnorm(3, 3, 1)/2), label = TeX('$1-\\beta$'), size = 9)+
  scale_x_continuous(breaks=c(0, qnorm(1-alpha), 3),
        labels=c(TeX('$\\mu_0$'), TeX('$Z_{1-\\alpha}$'), TeX('$\\mu > \\mu_0$')))+
  ylab('Probability Density')+
  xlab('')+
...
```
* in the plot above, the green region represents the probability of committing a type II error, the orange region the probability of committing a type I error and the pink/red region is the power of the test. 


**Below** the three panels show the relationship between power, the probability of a type I error and the probability of a type II error.
```{r, echo=F, message=F, warning=F, fig.height=6, fig.width=10}
significance.level = seq(0.001, 0.5, 0.001)
t = qnorm(1-significance.level)
beta = pnorm(t, mean = 1)
power = 1-beta
df = cbind.data.frame(significance.level, t , beta, power)

A=ggplot(data = df, aes(x = significance.level, y = power))+geom_line(size = 1)+
  theme_hc()+
...
B=ggplot(data = df, aes(x = beta, y = power))+geom_line(size = 1)+
  theme_hc()+
  xlab(TeX("Type II Error $\\beta$"))+
  ylab("Power")+
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 16))

C=ggplot(data = df, aes(x = beta, y = significance.level))+geom_line(size = 1)+
  theme_hc()+
  xlab(TeX("Type II Error $\\beta$"))+
  ylab(TeX("Significance level $\\alpha$"))+
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 16))
