...
header-includes:
  \usepackage{float}
  \usepackage{tikz}
bibliography: misc/Week_13_bibs.bib
urlcolor: blue
---
...
# Lecture 27 Monday, April 8th 2024
  
### Comparing two means

Last week we learned how to compare two populations across a qualitative response variable by comparing their relative proportions. Now we will learn how we can compare two groups on a quantitative response variable by comparing their means. There are many cases where researchers might be interested in comparing groups across a quantitative variable. For example, a teacher may be interested in comparing the average test scores of students in two different classes (Class A and Class B). A social scientist may be interested in whether there is a difference in salaries between men and women within a company. In any case, we are again dealing with two samples of sizes $n_1$ and $n_2$ which come from populations <i>population 1</i> and <i>population 2</i> and with means as $\mu_1$ and $\mu_2$. 
...
Just as it was for a single mean, estimating the population variances with the sample variances adds additional uncertainty. Thus, the sampling distribution for the difference in two means is $t$-distributed, although the degrees of freedom are calculated differently. The actual formula for the degrees of freedom is quite messy and difficult to compute:
...
\[\frac{\hat{\mu}_d - \mu_d}{SE(\hat{\mu}_d)} \sim t(\min(n_1 - 1, n_2 - 1)) \]

The margin of error for estimating the mean difference at the $100(1-\alpha)\%$ level is given by
...
\[\hat{\mu}_d = 4.2 - 3.8 = 0.4 \]

Since the confidence level is $95\%$ the standard score will be approximately the $t_{1-0.05/2} = t_{0.975}$ or the $97.5th$ percentile of a $t$-distribution with $173$ degrees of freedom. Since the $t$ and $z$ distributions are approximately equal for sample sizes $\geq 30$ we will use the standard score $1.96$. The standard error of the difference is given by 
...
$s_{pooled}$ is called the **pooled estimator** of the common standard deviation $\sigma$ because it combines the information from both samples.
...
**Example: NCAA Women's Basketball** - Sunday April 7th South Carolina and Iowa contended for the NCAA women's basketball championship. Consider the following table which gives the opponent and number of points scored for both teams in their last $15$ games leading up to the championship.

```{r, echo=FALSE, warning=F, message=F}
set.seed(123)

sc.matchup = rev(c('UConn','Tennessee','Georgia','Alabama', 'Kentucky', 'Arkansas', 'Tennessee', 'Texas A&M','Tennessee', 'LSU','Presbyterian', 'UNC', 'Indiana', 'Oregon State','NC State'))
...
Conduct an unpooled two sample $t$-test at the $\alpha = 0.05$ significance level to determine if there is a significant difference between the championship teams in the mean number of points scored per game.

  1. Let $\mu_1$ be the mean number of points scored per game for Iowa and $\mu_2$ be the mean number of points scored per game for South Carolina. The null hypothesis is that there is no difference in the mean number of points between the two teams 
  ...
  4. Based on the $p$-value above we would narrowly reject the null hypothesis and conclude that there is a significant difference in the mean number of points scored per game between Iowa and South Carolina. The outcome of the championship was an 87 - 75 victory for South Carolina. Given the conclusion of our test, this is a testament to South Carolina's defensive prowess in being able to shut down a high scoring team like Iowa. 


# Lecture 28 Wednesday, April 10th 2024

### Paired $t$ test
Comparative methods (i.e methods that compare two groups/samples across a quantitative variable) are more common and often preferred over one sample procedures. One common comparative design that makes use of single-sample procedures for a population mean is called the paired $t$-test. This test is used to draw inferences about a population mean in matched-pairs designs and can be thought of as the parametric analogue of the <i>sign test</i> we learned previously. Recall that in matched-pairs studies subjects are matched based on common characteristics and the quantitative outcome is compared within each pair. 
...
In a paired $t$-test we convert a comparative study into a one-sample procedure by making the response variable the difference between the two observations in each pair $d_i = x_i - y_i$. The parameter under study is the population mean difference $\mu_d$ which can be estimated by the sample mean difference $\bar{x}_d = \bar{x} - \bar{y}$ computed over all pairwise differences. The standard deviation of the difference $\sigma_d$ is then estimated with the sample standard deviation of the difference. Inference is then treated in the same way as one-sample procedures for a population mean
  ...
  * the $100(1 - \alpha)\%$ confidence interval for the mean pairwise difference is
  
  \[ \bar{x}_d \pm t_{1-\alpha/2} \frac{s_d}{\sqrt{n}}\]
  
  * the test statistic for a significance test regarding the pairwise difference is given by
...
**Example: Revisiting the Spousal Dominance Problem** - Recall the study comparing the ratings of husbands and wives on the perceived relative influence of each member of the couple on a major financial decision.
...
df = cbind.data.frame(`Parameter` = c('$p$','$p_1 - p_2$', '$\\mu$', '$\\mu_1 - \\mu_2$','$\\mu_1 - \\mu_2$', '$\\mu_d$'),
                      ...
                      `Response Variable` = c('Categorical','Categorical', 'Quantitative', 'Quantitative', 'Quantitative','Quantitative'),
                      
                      `$100(1 - \\alpha)\\%$ CI` = c('$\\hat{p}\\pm Z_{1-\\alpha/2}\\sqrt{\\frac{\\hat{p}(1-\\hat{p})}{n}}$',
                                                  ...
                                                  '$(\\bar{x}_1 - \\bar{x}_2) \\pm  t_{1-\\alpha/2, n_1+n_2 - 2} s_{pooled}\\sqrt{\\frac{1}{n_1}+\\frac{1}{n_2}}$',
...
\[X^2 = \sum_{i =1}^k \frac{(\text{observed count}_i - \text{expected count}_i)^2}{\text{expected count}_i} \]
  ...
  * We can think of this calculation as measuring the total (relative) squared distance from the observed model to the model we defined under the null hypothesis.
...
# Lecture 29 Friday, April 12th 2024
...
df = cbind.data.frame(Flavor = c('Garlic Blast','Bacon Delight','Chicken Crunch'),
                      `Observed Count` = c(22, 30, 23),
                      `Expected Count` = c('','',''))

kable(df, format = 'html', digits = 2,
      row.names = F, booktabs = F, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria", font_size = 12)%>%kable_styling(bootstrap_options = 'striped')
...
Note that the GOF test doesn't tell us anything about the range of plausible values for the frequency parameters.

### Measuring statistical association in two categorical variables
...
The $\chi^2$ test of independence is used to examine the relationship between two categorical variables in a two-way table (also called a **contingency table**). Consider the following hypothetical table which compares the categorical variable $A$, which has two categories, with the categorical variable $B$ which has three categories.

```{r, echo=F, message=F, warning=F}

df = rbind.data.frame(c('','', '', 'Variable A', ''),
                      c('','','Category 1', 'Category 2', 'Marginal Distribution B'),
                      cbind.data.frame(` ` = c('','Variable B','',''), 
                                       ` ` = c('Category 1','Category 2','Category 3','Marginal Distribution A'), 
                      `Category 1` = c('$P(\\text{Category} \\ 1A | \\text{Category} \\ 1B)$',
                                       '$P(\\text{Category} \\ 1A | \\text{Category} \\ 2B)$',
                                       '$P(\\text{Category} \\ 1A | \\text{Category} \\ 3B)$',
                                       '$\\sum_i P(\\text{Category} \\ 1A | \\text{Category} \\ iB)$'),
                      
                      `Category 2` = c('$P(\\text{Category} \\ 2A | \\text{Category} \\ 1B)$',
                                       '$P(\\text{Category} \\ 2A | \\text{Category} \\ 2B)$',
                                       '$P(\\text{Category} \\ 2A | \\text{Category} \\ 3B)$',
                                       '$\\sum_i P(\\text{Category} \\ 2A | \\text{Category} \\ iB)$'),
                      
                      `Marginal B` = c('$\\sum_j P(\\text{Category} \\ jA | \\text{Category} \\ 1B)$',
                                       '$\\sum_j P(\\text{Category} \\ jA | \\text{Category} \\ 2B)$',
                                       '$\\sum_j P(\\text{Category} \\ jA | \\text{Category} \\ 3B)$',
                                       '1')))

kable(df, format = 'html', digits = 2, col.names = c('','','','',''),
      row.names = F, booktabs = F, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria", font_size = 12)%>%kable_styling(bootstrap_options = 'striped')

```

The two-way table above helps us to break down the relationship between the outcomes of <i>Variable A</i> and the outcomes of <i>Variable B</i>. As we can see from the table, the cells give the conditional probabilities (or more often frequencies/counts) of each category of $A$ across each category of $B$. There are also usually row and column totals displayed in a two-way table which summarize the marginal distributions of each variable. 

A **$\chi$-square test of independence** takes advantage of the two-way table above to determine if the conditional distribution of the response variable (the columns) changes across the categories of the explanatory variable (the rows). In the example above, we can see that each row gives the conditional distribution of the response variable for a given category of the explanatory variable. If *variable A* and *variable B* are independent then the rows of the table should be more or less the same across all categories of the explanatory variable. If the outcomes of *variable A* depend on the outcomes of variable $B$, then the rows of the table will change as we move across the categories of *variable B*.
...
\[ X_{obs}^2 = \sum_{i=1}^r\sum_{j=1}^c \frac{(o_{ij} - e_{ij})^2}{e_{ij}}  \]
...
df <- data.frame(
  ` ` = c("Category 1", "Category 2", "⋮", "Category r", "Total"),
  `Category 1` = c("$o_{11}$", "$o_{21}$", "$\\vdots$", "$o_{r1}$", "$\\sum_{i=1}^r o_{i1}$"),
  `Category 2` = c("$o_{12}$", "$o_{22}$", "$\\vdots$", "$o_{r2}$", "$\\sum_{i=1}^r o_{i2}$"),
  `⋯` = c("$\\cdots$", "$\\cdots$", "$\\vdots$", "$\\cdots$", "$\\cdots$"),
  `Category c` = c("$o_{1c}$", "$o_{2c}$", "$\\vdots$", "$o_{rc}$", "$\\sum_{i=1}^r o_{ic}$"),
  `Total` = c("$\\sum_{j=1}^c o_{1j}$", "$\\sum_{j=1}^c o_{2j}$", "$\\vdots$", "$\\sum_{j=1}^c o_{rj}$", "$n = \\sum_{i=1}^r\\sum_{j=1}^c o_{ij}$")
)

kable(df, format = 'html', digits = 2, caption = 'The $r \\times c$ table of observed counts',
      row.names = F, booktabs = F, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped')
...
The test statistic follows a $\chi^2$ distribution with $(r-1)\times (c-1)$ degrees of freedom, where $r$ is the total number of row categories and $c$ is the total column categories. Let's try it out on an example:
...
df = cbind.data.frame(
  Income = c("Above Average", "Average", "Below Average", "Total"),
  `Not Too Happy` = c(29, 83, 104, 216),
  `Pretty Happy` = c(178, 494, 314, 986),
  `Very Happy` = c(135, 277, 119, 531),
  Total = c(342, 854, 537, 1733)
)
...
row_totals <- observed$Total[1:3]
col_totals <- c(sum(observed$`Not Too Happy`[1:3]), sum(observed$`Pretty Happy`[1:3]), sum(observed$`Very Happy`[1:3]))
...
for (i in 1:3) {
  for (j in 1:3) {
    expected[i, j] <- paste0('$\\frac{', row_totals[i], '\\times ', col_totals[j],'}','{',grand_total, '} = ',round((row_totals[i] * col_totals[j]) / grand_total, 2), '$')
  }
}

# Convert matrix to data frame for kable
expected_df <- as.data.frame(expected)
colnames(expected_df) <- c("Not Too Happy", "Pretty Happy", "Very Happy")

# Add row names
rownames(expected_df) <- c("Above Average", "Average", "Below Average")


kable(expected_df, format = 'html', digits = 2, caption = 'The $r \\times c$ table of expected counts',
      row.names = T, booktabs = F, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped')
...
differences <- matrix(nrow = 3, ncol = 3)
for (i in 1:3) {
  for (j in 1:3) {
    differences[i,j] = paste0('$\\frac{(', round(expected_df[i,j],2), ' - ', round(observed[1:3, 2:4][i,j],2), ')^2}{', round(expected_df[i,j],2), '} = ', round((expected_df[i,j] - observed[1:3, 2:4][i,j])^2/expected_df[i,j], 2), '$')
  }
}

differences_df = as.data.frame(differences)
colnames(differences_df) <- c("Not Too Happy", "Pretty Happy", "Very Happy")

# Add row names
rownames(differences_df) <- c("Above Average", "Average", "Below Average")


kable(differences_df, format = 'html', digits = 2, caption = 'The $r \\times c$ table of differences',
      row.names = T, booktabs = F, escape = F)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped')
...
which follows a $\chi^2$ distribution with $(3-1)\times (3 - 1) = 4$ degrees of freedom. The $p$-value is the probability of observing a value more extreme than $X_{obs}^2 = 54.04$ which is
